...
export const createSettlement = async (req, res) => {
  try {
    ...
    await sendNotification({
      ...
      message: `You received a settlement of ${amount}`,
      data: { settlementId: settlement._id },
    });
    ...
    return res
      .status(500)
      .json({ success: false, msg: "Failed to create settlement" });
  }
};
...
export const getGroupSettlements = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      msg: "Settlements fetched successfully",
      data: settlements,
    });
  } catch (e) {
    return res
      .status(500)
      .json({ success: false, msg: "Failed to fetch settlements" });
  }
};
