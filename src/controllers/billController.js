...
export const createBill = async (req, res) => {
  try {
    const { title, amount, group, paidBy, participants, involved, splitType, shares = [], items = [], note, category } = req.body;

    if (!title || !amount || !group || !paidBy || !participants || participants.length < 1) {
    ...
    switch (splitType) {
      case "equal":
        rawSplits = calculateEqualSplits({ amount, payerId: paidBy, participantIds: involved?.length ? involved : participants });
        break;
      case "unequal":
        rawSplits = calculateUnequalSplits({ payerId: paidBy, shares });
        break;
      case "shares":
        rawSplits = calculateShareWeightSplits({ amount, payerId: paidBy, shares });
        break;
      case "itemized":
      ...
      case "custom":
        ...
        rawSplits = shares.map((s) => ({ from: s.from, to: s.to, amount: s.amount }));
        break;
      default:
        return res.status(400).json({ success: false, message: "Invalid splitType" });
    }
    ...
    await Activity.create({ group, actor: paidBy, type: "bill_added", summary: `Added bill ${title}`, data: { billId: bill._id } });
    // notify participants
    for (const uid of participants) {
      if (String(uid) === String(paidBy)) continue;
      await sendNotification({ user: uid, type: "bill_added", title: "New bill added", message: title, data: { billId: bill._id, group } });
    }
