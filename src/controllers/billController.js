...
export const getMyTransactions = async (req, res) => {
  try {
    ...
    const bills = await Bill.find({ group: { $in: groupIds } })
      .select("title amount shares createdAt group")
      .populate("group", "name avatar")
      .sort({ createdAt: -1 })
      .lean();

    // Filter bills to get only the user's share
    ...
    for (const bill of bills) {
      ...
      if (userShare && userShare.amount > 0) {
        myTransactions.push({
          ...
          amount: Number(userShare.amount.toFixed(2)),
          timing: bill.createdAt,
          groupName: bill.group?.name || "Unknown Group",
          groupId: bill.group?._id || bill.group,
          groupIcon: bill.group?.avatar || "",
        });
      }
    }
